...
Elm.Counter.make = function (_elm) {
   ...
   var view = function (model) {
      ...
      _L.fromArray([$Html.text($Basics.toString(model.value))]));
   };
   var viewWithRemove = F2(function (model,
   remove) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([view(model)
                   ,A2($Html.button,
                   _L.fromArray([A2($Html$Events.onClick,
                   remove,
                   {ctor: "_Tuple0"})]),
...
Elm.CounterPair.make = function (_elm) {
   ...
   var view = function (model) {
      return A2($Html.div,
      _L.fromArray([]),
      _L.fromArray([$Counter.view(A2($Ref.field,
                   model,
                   "topCounter"))
                   ,$Counter.view(A2($Ref.field,
                   model,
                   "bottomCounter"))]));
   };
...
Elm.Ref = Elm.Ref || {};
Elm.Ref.make = function (_elm) {
   "use strict";
   _elm.Ref = _elm.Ref || {};
   if (_elm.Ref.values)
   return _elm.Ref.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   $moduleName = "Ref",
   $Basics = Elm.Basics.make(_elm),
   $Native$Ref = Elm.Native.Ref.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var fromMailbox = function (m) {
   ...
   var signal = function ($) {
      return fromMailbox($Signal.mailbox($));
   };
   var fieldSpec = $Native$Ref.fieldSpec;
   var FieldSpec = F2(function (a,
   b) {
      return {_: {},get: a,set: b};
   });
   var transform = function (r) {
   ...
   var field = F2(function (r,
   name) {
      return function () {
         var f = fieldSpec(name);
         return {_: {}
   ...
   var Ref = F2(function (a,b) {
      return {_: {}
             ,address: b
             ,value: a};
   });
   _elm.Ref.values = {_op: _op
                     ,Ref: Ref
                     ,set: set
                     ,transform: transform
                     ,FieldSpec: FieldSpec
